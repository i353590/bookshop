...
async function serve (all=[], o={}) { // NOSONAR
  ...
  // if started with ts-node, look for a server.ts file first
  const serverFiles = ['server.js']
  if (process.env.CDS_TYPESCRIPT === 'true')  serverFiles.unshift('server.ts')
  const serverFile = serverFiles.map (f => _local(f) || _local(cds.env.folders.srv,f)) .find (f => !!f)
  const serverImpl = serverFile ? require (serverFile) : cds.server  // must not be loaded unnecessarily
  if (typeof serverImpl !== 'function')  throw cds.error(cds.utils.local(serverFile)+' must export a function')
  const server = await serverImpl(o)
...
function _prepare_logging (o) { // NOSONAR
  ...
  cds.on ('serving', (srv) => {
    const details = { at: srv.path }, {_source} = srv
  ...
  cds.once ('listening', ({server, url})=>{
    ...
    if (server._source) { // log custom server.js/ts
      console.log ('[cds] - server loaded from', {file: local(server._source)})
    }
