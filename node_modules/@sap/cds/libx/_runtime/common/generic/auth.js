...
const _resolveUserAttrs = (restrict, req) => {
  ...
  while (next !== null) {
    ...
    restrict.where = restrict.where.replace(next[0], val)
...
const _enhanceAnnotationWhere = (query, where, model) => {
  query = cqn2cqn4sql(query, model, true)
...
const _getCapabilitiesHandler = (entity, annotation, srv) => {
  ...
  const _isNavigationRestricted = (target, path) => {
    if (!target) return
    ...
    if (target && Array.isArray(target['@Capabilities.NavigationRestrictions.RestrictedProperties'])) {
      for (const r of target['@Capabilities.NavigationRestrictions.RestrictedProperties']) {
        if (r.NavigationProperty['='] === path && r[parts[0]] && r[parts[0]][parts[1]] === false) return true
      }
    }
  }
  ...
  const handler = function (req) {
    ...
    if (nav.length > 1) {
      ...
      if (_isNavigationRestricted(target, path)) {
        // REVISIT: rework exception with using target
        const trgt = `${_localName(target)}.${path}`
        req.reject(405, 'ENTITY_IS_NOT_CRUD_VIA_NAVIGATION', [_localName(entity), action.toUpperCase(), trgt])
      }
    } else if (
      ...
      req.reject(405, 'ENTITY_IS_NOT_CRUD', [_localName(entity), action.toUpperCase()])
    }
  }
