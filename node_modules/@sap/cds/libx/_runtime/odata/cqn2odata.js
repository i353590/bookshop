...
const _formatVal = (val, element, csnTarget, kind) => {
...
const _format = (cur, element, target, kind, isLambda) => {
  if (typeof cur !== 'object') return _formatVal(cur, element, target, kind)
  if (hasValidProps(cur, 'ref')) return isLambda ? [LAMBDA_VARIABLE, ...cur.ref].join('/') : cur.ref.join('/')
  if (hasValidProps(cur, 'val')) return _formatVal(cur.val, element, target, kind)
...
const _keysOfWhere = (where, kind, target) => {
  ...
  for (const cur of where) {
    if (hasValidProps(cur, 'ref')) {
      ...
      res.push(_formatVal(cur.val, element, target, kind))
    } else if (cur === 'and') {
      res.push(',')
    } else {
      res.push(cur)
...
function $search(search, kind) {
  // odata-v2 does not support $search
  if (kind === 'odata-v2') return ''

  const expr = parseSearch(search).join(' ').replace('( ', '(').replace(' )', ')')
  return expr ? `$search=${expr}` : ''
}
...
function cqn2odata(cqn, kind, { safeNumber }, model) {
